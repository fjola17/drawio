...
function redrawCanvas(item){
    switch(drawio.selectedShape){
        case drawio.availableShapes.RECTANGLE:
            drawio.selectedElement = new Rectangle({ x: item.position.x, y: item.position.y }, item.width, item.height, item.strokeStyle, item.checked, item.lineWidth);
            break;
        ...
        case drawio.availableShapes.CIRCLE:
            drawio.selectedElement = new Circle({ x: item.position.x, y: item.position.y }, item.rad, item.strokeStyle, item.checked, item.lineWidth);
            break;
